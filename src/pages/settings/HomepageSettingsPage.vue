<template>
  <div>
    ...
    <q-card flat bordered>
      <q-card-section>
        <div class="text-subtitle1 text-weight-medium">{{ t('remaining.k121') }}</div>
        <div class="text-grey-7 text-caption q-mt-xs">{{ t('remaining.k122') }}</div>
      </q-card-section>
      <q-card-section>
        <div v-if="loading" class="flex flex-center q-pa-md">
          <q-spinner color="primary" />
        </div>
        <q-card v-else-if="!items.length" flat bordered class="q-pa-md text-grey-7 text-center">{{ t('remaining.k123') }}</q-card>
        <q-list v-else bordered separator class="rounded-borders">
          <q-item v-for="(item, index) in items" :key="item.id">
            <q-item-section>
              <q-item-label>{{ displayTitle(item.title) }}</q-item-label>
              <q-item-label caption>{{ t('extra.homepageOrder', { order: index + 1, id: item.id }) }}</q-item-label>
            </q-item-section>
            <q-item-section side>
              <div class="row no-wrap q-gutter-xs">
                ...
                >
                  <q-tooltip>{{ t('remaining.k124') }}</q-tooltip>
                </q-btn>
              </div>
            </q-item-section>
          </q-item>
...
async function load() {
  ...
  try {
    ...
    error.value = getErrorMessage(err, t('remaining.k118'))
  } finally {
    loading.value = false
  }
}
...
async function persist(nextItems) {
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: t('remaining.k119') })
  } catch (err) {
    items.value = previous
    error.value = getErrorMessage(err, t('remaining.k120'))
  } finally {
    saving.value = false
  }
}
