...
- 글 목록은 페이지 단위(기본 10개, 10/20/50/100 선택). 페이지는 `?page=`. 블로그 홈 페이지네이션은 피드 맨 아래에만 표시됩니다.
...
- 카테고리 트리를 오른쪽에 둘 때 기본 열림은 사이트 관리 → 일반의 `rightMenuDefaultOpen`.
...
- 사이트 관리(`/settings`)는 일반·카테고리·홈페이지·블로그·간단 포스트·첨부파일·데이터관리 메뉴로 나뉩니다.
- 블로그 모드와 한 페이지 글 수는 사이트 관리 → 블로그에서 설정합니다.
...
- Markdown 코드 블록 라인 번호는 사이트 관리 → 일반의 `codeLineNumbers`로 켜고 끕니다.
- 사이트 관리에서 파비콘을 바꿀 수 있습니다. **기본값으로 되돌리기**는 바로 저장됩니다.
- 첨부 파일 최대 용량(MB)과 미연결 파일 정리는 사이트 관리 → 첨부파일에서 합니다.
- 데이터관리(`/settings/data`)에서 백업·복구(`.wkmbak`)와 링크 미리보기 캐시 삭제를 합니다. 복구 전 파일 검사 후 확인 대화상자로 덮어씁니다.
